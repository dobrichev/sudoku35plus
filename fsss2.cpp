...
unsigned long long fsss2::solve(const int* const in, const unsigned long long nSolutions, char* const out) {
	//puzzle = in;
	//start from clean solver context
	initEmpty();
	sol = out;
	numSolutionsToDo = nSolutions;
	//perform optimized setup with the initial given pencilmarks
	for(int c = 0; c < 81; c++) {
		int d = in[c];
		if(d == 511) {
			//skip non-givens
