...
*******************************************************************************/
static int BlockScore (int m[9], const int * restrict perm1, const int * restrict perm2)
{ 
  int score, *r1, *r2, *r3;
...
*******************************************************************************/
static void AnswerInit (Answer_t * restrict ans)
{
  int i;
...
*/
static int WhizBang (Answer_t * restrict stk, char * restrict twopuz[2])
{
	...
	for ( kk=0; kk < savelen; kk++ ) {
		...
		ans->type = (unsigned short)type;
		ans->sord[0]  = (unsigned short)(3*perm2[0]);
		ans->sord[1]  = (unsigned short)(3*perm2[1]);
		ans->sord[2]  = (unsigned short)(3*perm2[2]);
		ans->rmap2[0] = (unsigned short)(j = (perm1[0]+k));	ans->rmap1[j] = 0;
		ans->rmap2[1] = (unsigned short)(j = (perm1[1]+k));	ans->rmap1[j] = 1;
		ans->rmap2[2] = (unsigned short)(j = (perm1[2]+k));	ans->rmap1[j] = 2;
	}
...
*******************************************************************************/
static int AttachRownum (Answer_t * restrict oldstk, int oldlen, int r2)
{
	Answer_t  *ans, *alt;
	...
	for ( newlen=oldlen, ii=0; ii < oldlen; ii++ )
	{
		...
		for ( first=99, r1=r1min; r1 < r1max; r1++ )
		{
			...
			{ /* Create alternate answer. */
				...
				alt->rmap1[r1] = (short)r2;
				alt->rmap2[r2] = (short)r1;
			}
		}

		/* Map first answer */
		r1 = first;
		ans->rmap1[r1] = (short)r2;
		ans->rmap2[r2] = (short)r1;  
	}
...
/*******************************************************************************
*******************************************************************************/
static int NextPerm (int * restrict perm, int len)
{
  /* *** Note:  0 <= len <= 20 *** */
...
/*******************************************************************************
*******************************************************************************/
static int CompareKeys (const int * restrict a, const int * restrict b)
{
  if (a[0] < b[0]) return -1;
  if (a[0] > b[0]) return +1;
  if (a[1] < b[1]) return -1;
...
/*******************************************************************************
*******************************************************************************/
static int ExtendAnswer (Answer_t * restrict ans, int pass,
						 Answer_t * restrict stk, int stklen, int topkey[3], char * twopuz[2])
{
	Answer_t *alt;
	int compare;
...
jump_in:
		...
		for ( ii=0; ii < clen1; ii++ ) {
			...
			for ( tt=t2-1; tt >= t1; tt-- )
				if ( alt->cmap2[tt] < 0 )
					break;     
			alt->cmap1[kk] = (short)tt;
			alt->cmap2[tt] = (short)kk;
		}
		...
		for ( ii=0; ii < 3; ii++ )
		{
			...
			/* Record info */
			key[ii] = s2;
			alt->smap1[s1] = (short)s2;
			alt->smap2[s2] = (short)s1;
		}
...
void subCanoner::canon(const char* restrict puz, char* restrict can) restrict {
	/* Declarations */
...
//	//fprintf(stderr, "/nmaxLen=%d/n", maxLen); //debug
//}
